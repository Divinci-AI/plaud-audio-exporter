...
// Download audio from share links
ipcMain.handle('download-share-audio', async (event, urls) => {
  try {
    const { batchDownload } = require('./plaude-html-link-downloader/share-downloader');
    
    // Create downloads directory
    const downloadsDir = path.join(settings.downloadDir, 'share-downloads');
    ...
    // Process URLs one by one to send progress updates
    for (let i = 0; i < urls.length; i++) {
      ...
      const { downloadAudioFromShareLink } = require('./plaude-html-link-downloader/share-downloader');
      const result = await downloadAudioFromShareLink(urls[i], downloadsDir);
      results.push(result);
    ...
    log.error('Error downloading share audio:', error);
    throw error;
  }
});
